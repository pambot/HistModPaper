...
for cell1 in cells:
    for cell2 in cells:
        if cell1 == cell2:
            if residuals:
                rgr_scores = pickle.load(open(folder + 'results/rgrExpr_res_'+cell1+'_0.0.pkl', 'rb'))
            else:
                rgr_scores = pickle.load(open(folder + 'results/rgrExpr_full_'+cell1+'_0.0.pkl', 'rb'))
...
if residuals:
    m_matrix.to_csv(folder + 'results/rgrExprValid_res.heatmap', sep='\t', header=True, index=True)
else:
    m_matrix.to_csv(folder + 'results/rgrExprValid_full.heatmap', sep='\t', header=True, index=True)
