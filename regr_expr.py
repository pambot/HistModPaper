...
for ttype in ['full', 'res']: 
    ...
    plt.savefig(folder + 'figures/rgrExpr_'+ttype+'_'+cell+'_'+str(threshold)+'.png', bbox_inches='tight')
    plt.clf()
    """
    
    saveres = {'r_value':r_value, 'slope':slope, 'intercept':intercept, 
        'p_value':p_value, 'stderr':stderr, 'coefs':coefs}
    pickle.dump(saveres, open(folder + 'results/rgrExpr_'+ttype+'_'+cell+'_'+str(threshold)+'.pkl', 'wb'))

print 'Done Expr', folder+cell
